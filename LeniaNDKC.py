...
try:
    import reikna.fft as reikna_fft
    import reikna.cluda as reikna_cluda
except ImportError:
    reikna_fft = None
    reikna_cluda = None
...
import warnings
from lenia_runtime import asset_path, ensure_runtime_file, runtime_path
...
class Automaton:
    ...
    def compile_gpu(self, A):
        ...
        if reikna_fft is None or reikna_cluda is None:
            self.has_gpu = False
            self.is_gpu = False
            return
        try:
...
class Lenia:
    ...
    SAVE_ROOT = runtime_path('save')
    ANIMALS_PATH = asset_path('animals.json' if DIM==2 else 'animals'+str(DIM)+'D.json')
    found_path = args.F
    if found_path is None:
        found_path = '{D}{C}{K}{X}.json'.format(D=DIM, C=CN, K=KN, X='' if XN==1 else str(XN))
    elif not found_path.endswith('.json'):
        found_path += '.json'
    FOUND_ANIMALS_PATH = ensure_runtime_file('found', found_path)
    ...
    def __init__(self):
        ...
        self.font = PIL.ImageFont.load(asset_path('resource', 'bitocra-13.pil'))
    ...
    def create_window(self):
        ...
        icon = tk.Image("photo", file=asset_path('resource', "icon"+str(icon_no)+".png"))
        ...
        self.canvas = tk.Canvas(self.frame, width=SIZEX*PIXEL, height=SIZEY*PIXEL, highlightthickness=0, bg='black')
    ...
    def get_canvas_size(self):
        ...
        if canvas_w <= 1 or canvas_h <= 1:
            return SIZEX*PIXEL, SIZEY*PIXEL
    ...
    def render_to_canvas(self):
        ...
        draw_img = self.img if (draw_w, draw_h) == self.img.size else self.img.resize((draw_w, draw_h), resample=PIL.Image.NEAREST)
        if self.img.mode == 'P' and draw_img is not self.img:
            draw_img.putpalette(self.img.getpalette())
        photo = PIL.ImageTk.PhotoImage(image=draw_img)
        self.panel_photo = photo
        self.canvas.itemconfig(self.panel1, image=photo)
        x0 = (canvas_w - draw_w) // 2
        y0 = (canvas_h - draw_h) // 2
        self.canvas.coords(self.panel1, x0, y0)
            ...
            if self.is_show_rgb():
                ...
                for c in CHANNEL:
                    ...
                    for val in range(ncol):
                        ...
                        yc0 = y0 + dy*val
                        yc1 = y0 + dy*(val+1)
                        draw.rectangle([(x0+c*3,min(yc0, yc1)),(x0+(c+1)*3-1,max(yc0, yc1))], fill=tuple(color))
            else:
                for c in range(ncol):
                    yc0 = y0 + dy*c
                    yc1 = y0 + dy*(c+1)
                    draw.rectangle([(x0,min(yc0, yc1)),(x1,max(yc0, yc1))], fill=self.get_color(c))
            draw.rectangle([(x0-1,min(y0+1, y1-1)),(x1+1,max(y0+1, y1-1))], outline=self.get_color(254))
...
def bootstrap_lenia(run_loop=True):
    lenia = Lenia()
    #lenia.print_help()
    is_loaded = False
    if lenia.ANIMAL_KEY_LIST != {} and lenia.ANIMAL_KEY_LIST['1'] != '':
        animal_id = lenia.load_animal_code(lenia.world, lenia.ANIMAL_KEY_LIST['1'])
        is_loaded = animal_id is not None and animal_id != -1
    if not is_loaded:
