...
import warnings
from lenia_runtime import asset_path, runtime_path
...
class Lenia:
    ...
    FOUND_ANIMALS_PATH = runtime_path('found.json')
    ...
    def draw_legend(self, markers=[]):
        ...
        if 'colormap' in markers and self.markers_mode in [0,1,4,5] and self.polar_mode in [0,1]:
            ...
            dy = (y1-y0)/253
            draw.rectangle([(x0-1,min(y0+1, y1-1)),(x1+1,max(y0+1, y1-1))], outline=254)
            for c in range(253):
                yc0 = y0 + dy*c
                yc1 = y0 + dy*(c+1)
                draw.rectangle([(x0,min(yc0, yc1)),(x1,max(yc0, yc1))], fill=c)
